...
// First have Young face all young, then young and grown face young and adult etc.  Where each phase if one team wins by more than a certain threshold then the game is over and that team won.
sealed trait Age {
  val phaseIndex: Int
}
...
case class AgeSynergy(basePower: Double, age: Age) extends Synergy {
  def power: Breakdown => Double = (breakdown: Breakdown) => breakdown.age.getOrElse(age, 0.0d) * basePower
}

case class Card(basePower: Double = 5.0, age: Age = Young, profession: Option[Profession] = None, race: Option[Race] = None, synergies: Set[Synergy], counters: Set[Synergy]) {
  def power: (Breakdown, Breakdown) => Double = (allyBreakdown: Breakdown, enemyBreakdown: Breakdown) => {
    basePower +
      synergies.foldLeft(0.0) { (pow, s) => pow + s.power(allyBreakdown) } +
      counters.foldLeft(0.0) { (pow, c) => pow + c.power(enemyBreakdown) }
  }
}
